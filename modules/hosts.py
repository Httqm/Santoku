...
class Host(object):
    ...
    def isMarkedToBeIgnored(self):
        return 1 if self.csv.getCurrentHostCell(config.csvHeaderIgnoreHost) == '1' else 0
    ...
    def isDuplicated(self):
        ...
        """
        import re
        match = re.search(self.csv.getCurrentHostCell(config.csvHeaderHostName), self.allHosts.output)
